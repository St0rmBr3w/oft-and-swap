...
task("sendAndSwap", "Calls the send function on the MyOFTMock contract with encoded swap parameters")
    ...
    .setAction(async (taskArgs: TaskArguments, { ethers }) => {
        ...
        const amountToSwap = ethers.utils.parseEther(taskArgs.amount);
        ...
        const sendParam: SendParam = {
            ...
            extraOptions: Options.newOptions().addExecutorLzReceiveOption(200000, 0).addExecutorComposeOption(0, 200000, 1000000000000000).toHex().toString(), // Assuming no extra options are needed
